...
public class JwtService {
    @Value("${broker.security.jwtSecretKey}")
    private String jwtSecretKey;
    @Value("${broker.security.jwtAccessTokenExpirationDuration}")
    private int jwtAccessTokenExpirationDuration;
    @Value("${broker.security.jwtRefreshTokenExpirationDuration}")
    ...
    private String buildToken(Map<String, Object> extraClaims, UserDetails userDetails, int expirationDuration) {
        ...
        extraClaims.put("jti", System.nanoTime() + "-" + Thread.currentThread().getId());

        return Jwts.builder()
                .claims(extraClaims)
                .subject(userDetails.getUsername())
                .issuedAt(new Date(System.currentTimeMillis()))
                .expiration(new Date(System.currentTimeMillis() + (long) expirationDuration * 60 * 1000))
